	...
	int main()
	{
		...
		for (auto mesh : loader.LoadedMeshes){
			for (int i = 0; i < mesh.Vertices.size(); i += 3){
				...
				for (int j = 0; j < 3; j++){
					obj.vertexs_.emplace_back(mesh.Vertices[i + j].Position.X, mesh.Vertices[i + j].Position.Y, mesh.Vertices[i + j].Position.Z);
					obj.texcoords_.emplace_back(mesh.Vertices[i + j].TextureCoordinate.X, mesh.Vertices[i + j].TextureCoordinate.Y);
				}
		...
		while (remaining_frames--) {
			...
			//draw_object_naive(scene, zbuffer, sample_buffer, ortho_matrix, obj);
